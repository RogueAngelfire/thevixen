...
function ProductScreen() {
  ...
  useEffect(() => {
    ...
    console.log(qty)
  }, [dispatch, id, qty, successProductReview])

  const addToBasketHandler = () =>{
    console.log('added to basket', id, 'id number', qty, 'quantity added')
    navigate(`/basket/${id}?qty=${qty}`)
  }
  /// Issue here with id as not working and old params linked to previous react router dom
  ...
  return (
    <div>
      ...
      <Row>
        <Col md={6}>
          <h4>Reviews</h4>
          {product.reviews.length === 0 && <Message variant='info'>No Submitted Reviews</Message>}
          <ListGroup variant='flush'>
            {product.reviews.map((review) => (
              <ListGroup.Item key={review._id}>
                <strong>{review.name}</strong>
                <Rating value={review.rating} color='#f88e825'/>
                ...
                <Form onSubmit={submitHandler}>
                  <Form.Group controlId='rating'>
                    ...
                    >
                      ...
                      <option value='4'>4 Exellent</option>
